...
# どんな論文なのか？
医用画像(3D)に対して表現学習を適用した論文。
これまでの表現学習のフレームワークは2Dやclassification (encoder)をメインターゲットにしていたが、
この論文では3Dやsegmentationをターゲットにした表現学習のフレームワークを提案した。
他の事前学習の手法やscratchからの学習よりもこの論文の提案手法(表現学習)のほうが精度が良くなることを確認した。
...
この論文では3Dの医用画像をターゲットにした新しいフレームワークを提案しており、EncoderとDecoderの両方を事前学習することができる。
